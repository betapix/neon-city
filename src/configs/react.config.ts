/**
 * Set up for improving bridge requests by Shopify
 * https://shopify.engineering/improving-shopify-app-s-performance
 * https://gist.github.com/naqvitalha/fafd4f7946b18ecfa6a86b5ba65088e2#file-batchedsetstate-ts
 */
 ...
 * Batching can significantly improve performance when multiple state updates
 * are required in rapid succession. Please note the setState returned by this
 * hook is stable but may still trigger exhaustive deps warnings. You can add it
 * to the deps array to silence the warning.
 */
export const useBatchedState = createBatchedStateHook(setTimeout);

/**
 * This exposes default `useState` implementation. Useful if batched state
 * updates are enabled globally which replaces the `useState` hook.
 */
